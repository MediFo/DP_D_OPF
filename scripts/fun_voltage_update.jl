"""
Voltage/angle update function for ADMM
Modified to support both HiGHS (free) and Gurobi (commercial) solvers
"""

# Solver configuration - CHOOSE ONE:
# Option 1: HiGHS (FREE, open-source) - DEFAULT
using HiGHS
const SOLVER_UPDATE = HiGHS.Optimizer

# Option 2: Gurobi (commercial, requires license) - COMMENTED OUT
# using Gurobi
# const SOLVER_UPDATE = Gurobi.Optimizer
...
function update_θ(gen,bus,line,B,refbus,μ,θ̅,ρ)
    ...
    Solver: HiGHS (free) by default
    To use Gurobi: Comment out HiGHS lines above, uncomment Gurobi lines
    """
    Ng = length(gen)
    Nb = length(bus)
    Nl = length(line)
