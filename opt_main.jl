"""
OPTIMIZED Boyd's Consensus ADMM with Paillier Encryption for DC-OPF
Uses SPARSE neighbor-only variables instead of dense Nb×Nb matrices

KEY OPTIMIZATION:
- Old: Creates θ[1:Nb, 1:Nb] = 196 variables for 14-bus system
- New: Creates θ[(i,j) for i in 1:Nb, j in neighbors[i]] = ~42 variables
- Result: ~78% fewer variables, much faster optimization!
"""
...
"""
OPTIMIZED: Encrypt sparse θ dictionary
"""
...
"""
OPTIMIZED: Homomorphic consensus with sparse encrypted values
"""
function update_θ̅_homomorphic_sparse(public_key, private_key, bus, θ_enc)
    Nb = length(bus)
...
println("\n" * "="^80)
println(" OPTIMIZED ADMM + PAILLIER (SPARSE NEIGHBOR-ONLY VARIABLES)")
println(" Reduces variables by ~70-80% for typical power networks!")
println("="^80 * "\n")
...
# ─────────────────────────────────────────────────────────────────────────────
println("\n[6/6] Running OPTIMIZED ADMM with Paillier encryption...")
println("\nDetailed timing: OPT=Optimization, CRYPTO=Cryptography, OTHER=Other")
println("\n" * "─"^110)
