"""
Centralized OPF function - Gurobi Solver Version
Uses Gurobi commercial solver for optimization
"""
...
function OPF_centralized(gen,bus,line,B,refbus,gurobi_env)
    ...
    Solver: Gurobi (commercial solver with license)
    ...
    Args:
        ...
        gurobi_env: Gurobi environment
    """
    Ng = length(gen)
    Nb = length(bus)
    Nl = length(line)

    # Create model with Gurobi solver
